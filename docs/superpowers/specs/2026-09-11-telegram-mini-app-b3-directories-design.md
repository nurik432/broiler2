...
- **Запись голосовых заметок на мобилке.** `useVoiceRecording` использует
  `MediaRecorder`/`getUserMedia` в браузере — поведение внутри Telegram
  WebView (особенно iOS) не проверено и не гарантировано. Мобильный экран
  заметок **показывает** уже существующие голосовые заметки (аудиоплеер,
  `audio_url` — будь то base64 data URI или Storage URL, `<audio>`
  воспроизводит оба без разницы), но не добавляет кнопку записи. Если
  понадобится — отдельный подпроект.
...
| Вопрос | Решение |
|---|---|
| Голосовые заметки на мобилке | Только воспроизведение существующих, без записи (см. «Вне рамок»). |
